@extends('layouts.app')
@section('title')
    Livrables({{ count($users) }})
@endsection
@section('filsAriane')
    <li class="breadcrumb-item"><a href="#">Livrables</a></li>
@endsection
@section('content')
                ...
                <tbody>
                    @forelse ($users as $user)
                        ...
                        <div class="modal fade" id="modal-default-{{ $user->id }}">
                            <form action="{{ route('approuving.update', $user->user_id) }}?response=refus&project={{ $user->project_id}}" method="post">
                                @csrf
                                @method('PUT')
                        ...
                        <div class="modal fade" id="modal-default-{{ $user->id }}-accepter">
                            <form action="{{ route('approuving.update', $user->user_id) }}?response=accepter&project={{ $user->project_id}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="modal-dialog">
                                    <div class="modal-content">
